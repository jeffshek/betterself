;(function(){
	...
	var getIamgeSource = function(fid , server , id , secret , size){
	...
	p._photosData = function(data){
		...
		$.each(data[this.options.type].photo, function(i,item){
			...
			//image
			slide_cont += '<img src="masterslider/blank.gif" data-src="' + getIamgeSource(item.farm , item.server , item.id , item.secret , that.options.imgSize) + '" alt="' + title + '" />';
			
			//thumb
			if(that.options.thumbs)
				slide_cont += '<img class="ms-thumb" src="' + getIamgeSource(item.farm , item.server , item.id , item.secret , that.options.thumbSize) + '" alt="' + title + '" />';
